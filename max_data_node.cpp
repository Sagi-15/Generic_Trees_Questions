...
TreeNode<int>* maxDataNode(TreeNode<int>* root){
    /*Logic pehle root walle ko max mann
    uske har subtree mei se joh bhi max woh store
    recursion laake dega woh aur compare uska data with previous joh
    max mana tha agar bada then max change aur return wahi*/
    TreeNode<int> *max_data_walla_node=root;
    /*Now store max from each subtree*/
    for(int i=0;i<root->children.size();i++){
        TreeNode<int> *smallAnswer=maxDataNode(root->children[i]);
        /*so small answer mei har loop mei subtree ka max aa*/
        /*compare iske data ko previous data se*/
        if(max_data_walla_node->data<smallAnswer->data){
            max_data_walla_node=smallAnswer;
        }
    }
    return max_data_walla_node;
}
int main(){
    TreeNode<int> *root=take_input_level_wise();
    printLevelWise(root);
    cout<<endl<<maxDataNode(root)->data;
}